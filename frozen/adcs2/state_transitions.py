...
def check_trans_angerr(data, trans_criteria : tp.trans_criteria_t, tmp1, tmp2, tmp3):
    # tmp1 = False
	tmp1 = False
	ang_err = np.arccos( (data.ekf_data.q[0])*(data.q_des[0]) + np.dot( (data.ekf_data.q[1:4]), (data.q_des[1:4])))
	print("(check_trans_angerr) ang_err: ")
	print(ang_err)
	
	if( tp.CHECK_MIN_ANG_ERR_TRANS_CRIT ): # check minimum angular error magnitude
		tmp1 = True
		try: ang_err = 360.0/pi*np.acos( round((data.ekf_data.q[0])*(data.q_des[0]) + np.dot( (data.ekf_data.q[1:4]), (data.q_des[1:4])) , 5) ) #[deg]
		except: ang_err = 360.0/pi*np.arccos( round((data.ekf_data.q[0])*(data.q_des[0]) + np.dot( (data.ekf_data.q[1:4]), (data.q_des[1:4])) , 5) ) #[deg]
	...
	if (tp.CHECK_MAX_ANG_ERR_TRANS_CRIT):
		try: ang_err = 360.0/pi*np.acos( (data.ekf_data.q[0])*(data.q_des[0]) + np.dot( (data.ekf_data.q[1:4]), (data.q_des[1:4])) )
		except: ang_err = 360.0/pi*np.arccos( (data.ekf_data.q[0])*(data.q_des[0]) + np.dot( (data.ekf_data.q[1:4]), (data.q_des[1:4])) )
...
# 	attempting EKF
    if( check_transition_criteria( sat.data, tp.Detumble2EKFRestart ) and 
       (mode == 2 or mode == 3) and (not sat.stayBdot)): ## if cdh wants adcs to stay in bdot for payload
