    ...
    if (((MICROPY_PY_BUILTINS_BYTEARRAY)
         || (MICROPY_PY_ARRAY
             && (mp_obj_is_type(args[0], &mp_type_bytes)
                 || (MICROPY_PY_BUILTINS_BYTEARRAY && mp_obj_is_type(args[0], &mp_type_bytearray)))))
...
STATIC mp_obj_t aes_xcrypt(mp_obj_t self_in, mp_obj_t buff)
{
	...
	size_t len=0;
    size_t sz=0;
	if (self->mode != 1) mp_raise_ValueError(MP_ERROR_TEXT("Cannot use xcrypt for given mode"));
	mp_buffer_info_t bufinfo;		
    if (((MICROPY_PY_BUILTINS_BYTEARRAY)
         || (MICROPY_PY_ARRAY
...
STATIC const mp_rom_map_elem_t aes_globals_table[] = {
...
MP_REGISTER_MODULE(MP_QSTR_aes, aes_user_cmodule, 1/*MODULE_PROPERTYCLASS_ENABLED*/);
